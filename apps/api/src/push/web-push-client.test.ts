...
describe("WebPushClient", () => {
  it("turns the real web-push rejection status into a dispatcher-visible result", async () => {
    ...
    await expect(client.send({ subscription: { endpoint: "https://push.example/subscription", p256dh: "p", auth: "a" }, payload: { type: "review_due", reminderId: "id", url: "/today?reminder=id" } })).resolves.toEqual({ statusCode: 410 });
  });
});
