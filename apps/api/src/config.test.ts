  ...
  it("requires the exact documented ALLOWED_ORIGIN", () => {
    ...
    expect(() =>
      loadConfig({
        ...
        ALLOWED_ORIGIN: "https://wrong.example",
      }),
    ).toThrow();
    expect(
      loadConfig({ ...productionEnvironment, ALLOWED_ORIGIN: PWA_ORIGIN })
        .allowedOrigin,
    ).toBe(PWA_ORIGIN);
  });
  ...
  it("validates listener and logging settings", () => {
    expect(
      loadConfig({
        ...
        PORT: "3040",
        LOG_LEVEL: "debug",
        DEADLINE_DELIVERY_LEAD_SECONDS: "120",
      }),
    ).toMatchObject({
      port: 3040,
      logLevel: "debug",
      deadlineDeliveryLeadSeconds: 120,
    });
    expect(() =>
      loadConfig({
        ...productionEnvironment,
    ...
    expect(() =>
      loadConfig({
        ...productionEnvironment,
        ALLOWED_ORIGIN: PWA_ORIGIN,
        DEADLINE_DELIVERY_LEAD_SECONDS: "-1",
      }),
    ).toThrow();
    expect(() =>
      loadConfig({
        ...productionEnvironment,
        ALLOWED_ORIGIN: PWA_ORIGIN,
        DEADLINE_DELIVERY_LEAD_SECONDS: "3601",
      }),
    ).toThrow();
  });
  ...
  it("requires the confirmed VAPID contact address", () => {
    expect(() =>
      loadConfig({
        ...productionEnvironment,
        ALLOWED_ORIGIN: PWA_ORIGIN,
        VAPID_SUBJECT: "mailto:other@example.com",
      }),
    ).toThrow();
    expect(
      loadConfig({ ...productionEnvironment, ALLOWED_ORIGIN: PWA_ORIGIN })
        .vapidSubject,
    ).toBe("mailto:gerupon@gmail.com");
  });
});
