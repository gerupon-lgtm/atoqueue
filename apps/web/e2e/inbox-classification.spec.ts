...
test("keeps a suggested capture unclassified until the user confirms タスクにする", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await expect(
    page.getByRole("heading", { name: "タスク候補を確認" }),
  ).toBeVisible();
  ...
  await expect
      ...
      page.evaluate(() => {
        ...
        return {
          ...
          linkedTaskMatchesCapture: snapshot.tasks.some(
            (task) =>
              task.id === capture?.linkedTaskId &&
              task.sourceCaptureId === capture.id,
          ),
        };
      }),
    )
    .toEqual({
      classification: "task",
      hasLinkedTask: true,
      linkedTaskMatchesCapture: true,
    });
});

test("keeps an unneeded capture recoverable until confirmed permanent deletion", async ({
  page,
}) => {
  await page.goto("/");
  await page
