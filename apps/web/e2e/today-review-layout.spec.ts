...
test("keeps the today review header, progress, and card inside the desktop content width", async ({ page }) => {
  ...
  for (const box of [header!, progress!, card!]) {
    expect(box.width).toBeLessThanOrEqual(736);
    expect(box.x).toBeGreaterThan(0);
    expect(box.x + box.width).toBeLessThan(1280);
  }
  for (const box of [header!, progress!]) {
    expect(Math.abs(box.x - card!.x)).toBeLessThan(2);
    expect(Math.abs((box.x + box.width) - (card!.x + card!.width))).toBeLessThan(2);
  }
  expect(header!.y + header!.height).toBeLessThanOrEqual(progress!.y);
