...
test("Settings exposes portable JSON export and an explicit restore control", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "JSONバックアップを書き出す" }),
  ).toBeVisible();
  await expect(page.getByLabel("JSONバックアップを復元")).toBeVisible();

  await page
    .getByRole("button", { name: "JSONバックアップを書き出す" })
    .click();
  await expect(
    page.getByRole("link", { name: "バックアップをダウンロード" }),
  ).toHaveAttribute("download", /atoqueue-backup-\d{4}-\d{2}-\d{2}\.json/);
});

test("F-017/F-018 restores an exported backup into a clean browser context after confirmation", async ({
  browser,
}) => {
  ...
  const sourceSnapshot = {
    ...
    device: {
      localDeviceId: "77777777-7777-4777-8777-777777777777",
    ...
    settings: {
      ...
      customTaskCategories: ["冷蔵庫"],
    },
    captures: [
      {
        id: "11111111-1111-4111-8111-111111111111",
        body: "clean context task",
        classification: "task",
        createdAt: "2026-08-04T09:00:00.000Z",
        updatedAt: "2026-08-04T09:00:00.000Z",
        classifiedAt: "2026-08-04T09:00:00.000Z",
        linkedTaskId: "22222222-2222-4222-8222-222222222222",
      },
    ],
    tasks: [
      {
        id: "22222222-2222-4222-8222-222222222222",
        sourceCaptureId: "11111111-1111-4111-8111-111111111111",
        title: "clean context task",
        category: "冷蔵庫",
        status: "active",
        dueMode: "none",
        nextReviewAt: "2026-08-04T09:00:00.000Z",
        undecidedCount: 0,
        dismissCount: 0,
        postponeCount: 0,
        createdAt: "2026-08-04T09:00:00.000Z",
  ...
  await sourcePage
    .getByRole("button", { name: "JSONバックアップを書き出す" })
    .click();
  const backup = await sourcePage
    .getByRole("link", { name: "バックアップをダウンロード" })
    .evaluate(async (link) => {
      return fetch((link as HTMLAnchorElement).href).then((response) =>
        response.text(),
      );
    });
  await source.close();
  ...
  await expect
    ...
    .toMatchObject({
      ...
      settings: expect.objectContaining({ customTaskCategories: ["冷蔵庫"] }),
    });
  const restoredDevice = await page.evaluate(
    () =>
      JSON.parse(window.localStorage.getItem("atoqueue:data:v1") ?? "{}")
        .device,
  );
  expect(restoredDevice.localDeviceId).not.toBe(
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return [restored.captures?.length, restored.tasks?.length];
      }),
    )
    .toEqual([1, 1]);
