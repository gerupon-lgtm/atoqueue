...
export function TaskListPage({ repository, now = () => new Date().toISOString() }: TaskListPageProps) {
  ...
  const tasks = useMemo(() => {
    ...
    return listTasks(snapshot.tasks, { tab, now: timestamp, calendar, ...(due ? { due } : {}), ...(category ? { category } : {}), search }, snapshot.captures);
  ...
  return <section aria-labelledby="task-list-title">
    ...
    <label>状態<select style={touchTarget} value={tab} onChange={(event) => setTab(event.target.value as TaskTab)}><option value="active">対応中</option><option value="completed">完了</option><option value="archived">アーカイブ</option></select></label>
...
function dueState(task: Task, now: string, calendar: { today(instant: string): string }): string {
  if (task.dueMode === "unset") return "期限未設定";
  if (task.dueMode === "none") return "期限なし";
  if (!task.dueAt) return "期限あり";
  if (task.dueAt < now) return "期限超過";
  return calendar.today(task.dueAt) === calendar.today(now) ? "今日が期限" : "期限あり";
}
