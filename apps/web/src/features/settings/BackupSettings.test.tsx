...
describe("BackupSettings", () => {
  ...
  it("F-018 previews incoming and current counts, then replaces only after explicit confirmation", async () => {
    ...
    expect(
      await screen.findByText(/取り込みデータ: タスク 1件/),
    ).not.toBeNull();
  ...
  it("keeps the destructive confirmation choices equally arranged", async () => {
    ...
    const confirmation = screen.getByRole("button", { name: "削除を確定する" });
    const actions = confirmation.closest(".backup-settings__delete-actions");
    expect(actions).not.toBeNull();
    expect(
      screen
        .getByRole("button", { name: "やめる" })
        .closest(".backup-settings__delete-actions"),
    ).toBe(actions);
  });
});
...
function example(): AppSnapshot {
  const value = createEmptySnapshot({
    appVersion: "0.1.0",
    localDeviceId: "99999999-9999-4999-8999-999999999999",
    timeZone: "Asia/Tokyo",
    now,
  });
  value.captures = [
...
function memory(
  value: AppSnapshot,
): AppRepository & { save: ReturnType<typeof vi.fn> } {
