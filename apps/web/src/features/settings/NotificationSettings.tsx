...
export function NotificationSettings({ repository, setup }: NotificationSettingsProps) {
  const [state, setState] = useState<NotificationSetupResult["state"]>();
  const [busy, setBusy] = useState(false);
  useEffect(() => { let active = true; void repository.load().then((snapshot) => { if (active) setState(snapshot.device.pushSubscriptionStatus === "not_requested" ? undefined : snapshot.device.pushSubscriptionStatus); }); return () => { active = false; }; }, [repository]);
  ...
  return <section aria-labelledby="notification-settings-title">
    ...
    <button disabled={busy} onClick={() => void configure()} type="button">通知を設定する</button>
