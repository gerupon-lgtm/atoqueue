import type { ActionEvent, ReviewCalendar, Task } from "../../../../../packages/domain/src";
...
export function createReviewPresentation(input: {
  ...
  if (input.task.dueMode === "none") return { deadline: "期限なし", elapsed: "週次確認" };
  if (input.task.dueMode === "unset") return { deadline: "期限未設定", elapsed: `次回確認: ${input.calendar.today(input.task.nextReviewAt)}` };
  if (!input.task.dueAt) return { deadline: "期限あり", elapsed: "期限を確認してください" };

  const date = input.calendar.today(input.task.dueAt);
  if (input.calendar.compareInstants(input.task.dueAt, input.now) < 0) {
    return { deadline: `期限: ${date}`, elapsed: `期限から${input.calendar.elapsedDays(input.task.dueAt, input.now)}日` };
  }
  return { deadline: `期限: ${date}`, elapsed: date === input.calendar.today(input.now) ? "期限は今日" : `期限まで: ${date}` };
}
...
export function latestSessionAnswer(input: {
  ...
  for (const id of [...input.actionEventIds].reverse()) {
    ...
    if (!event || event.entityType !== "task" || event.entityId !== input.taskId) continue;
    ...
    if (event.action === "task_rescheduled") {
      ...
      return typeof dueAt === "string" && dueAt === input.calendar.endOfDay(input.calendar.today(input.now))
        ? "今日やる"
        : "日付を変えた";
    }
  }
