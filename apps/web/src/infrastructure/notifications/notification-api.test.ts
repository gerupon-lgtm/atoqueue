...
const credentials = { deviceId: "11111111-1111-4111-8111-111111111111", deviceSecret: "device-secret" };
...
describe("NotificationApi", () => {
  it("sends an upsert made only from an outbox item and device credentials", async () => {
    const fetcher = vi.fn().mockResolvedValue(jsonResponse(201, {
      reminderId: "22222222-2222-4222-8222-222222222222", status: "pending", scheduledAt: "2026-08-04T09:00:00.000Z", updatedAt: "2026-08-04T08:00:00.000Z",
    }));
    const api = new NotificationApi("https://api.example.test", fetcher);

    await api.upsert({ id: "outbox", operation: "upsert", reminderId: "22222222-2222-4222-8222-222222222222", scheduledAt: "2026-08-04T09:00:00.000Z", notificationType: "task_review", taskRevision: 3, attemptCount: 0, nextAttemptAt: "2026-08-04T08:00:00.000Z", createdAt: "2026-08-04T08:00:00.000Z" }, credentials);

    expect(fetcher).toHaveBeenCalledWith("https://api.example.test/v1/reminders/22222222-2222-4222-8222-222222222222", expect.objectContaining({
      method: "PUT",
      headers: expect.objectContaining({ Authorization: "Bearer device-secret" }),
      body: JSON.stringify({ deviceId: credentials.deviceId, scheduledAt: "2026-08-04T09:00:00.000Z", notificationType: "task_review" }),
    }));
    expect(fetcher.mock.calls[0][1].body).not.toContain("task title");
  });

  it("reports retry metadata for a rate-limited request", async () => {
    const api = new NotificationApi("https://api.example.test", vi.fn().mockResolvedValue(new Response("", { status: 429, headers: { "Retry-After": "90" } })));
  ...
  it("registers once then updates an existing device subscription", async () => {
    const fetcher = vi.fn()
      ...
      .mockResolvedValueOnce(jsonResponse(200, { deviceId: credentials.deviceId, status: "active", updatedAt: "2026-08-04T08:00:00.000Z" }));
    const api = new NotificationApi("https://api.example.test", fetcher);
    ...
    expect(fetcher.mock.calls.map(([url, init]) => [url, (init as RequestInit).method])).toEqual([
      ...
      [`https://api.example.test/v1/devices/${credentials.deviceId}/subscription`, "PUT"],
    ]);
  });
});
