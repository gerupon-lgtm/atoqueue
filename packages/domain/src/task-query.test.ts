...
import { listTasks, type Task } from "./index";
...
describe("listTasks", () => {
  it("keeps the selected active, completed, or archived tab separate", () => {
    ...
    expect(listTasks(tasks, { tab: "active", now, today: "2026-08-03" }).map(({ id }) => id)).toEqual(["active"]);
    expect(listTasks(tasks, { tab: "completed", now, today: "2026-08-03" }).map(({ id }) => id)).toEqual(["completed"]);
    expect(listTasks(tasks, { tab: "archived", now, today: "2026-08-03" }).map(({ id }) => id)).toEqual(["archived"]);
  });
  ...
  it("filters active tasks by overdue, today, unset, none, and category", () => {
    ...
    const input = { tab: "active" as const, now, today: "2026-08-03" };
  ...
  it("finds a Unicode substring without changing the selected task state", () => {
    ...
    expect(listTasks(tasks, { tab: "active", now, today: "2026-08-03", search: "牛乳" }).map(({ id }) => id)).toEqual(["match"]);
    expect(listTasks([...tasks, task("old", { title: "牛乳", status: "completed", completedAt: now })], { tab: "active", now, today: "2026-08-03", search: "牛乳" }).map(({ id }) => id)).toEqual(["match"]);
  });
  ...
  it("sorts stably by next review, then due time, then creation time", () => {
    ...
    expect(listTasks(tasks, { tab: "active", now, today: "2026-08-03" }).map(({ id }) => id)).toEqual([
